...
func CreateUser(u *dto.User) error {
	...
	RETURNING id`
	id := ""
	err := db.QueryRow(sqlStatement, u.ID, u.Name, u.Email, u.ProfilePhotoURL, u.FirebaseUserID).Scan(&id)
	if err != nil {
...
func ReadUser(firebaseUserId string) (*dto.User, error) {
	...
	err := row.Scan(&u.ID, &u.Name, &u.Email, &u.ProfilePhotoURL, &u.FirebaseUserID)
	if err != nil {
